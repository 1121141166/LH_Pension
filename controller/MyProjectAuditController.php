...
class MyProjectAuditController extends BaseController {
    ...
    public function index(){
        ...
        if($_params['audit_status'] != ''){
            $where['project_nodes_audit.status'] = $_params['audit_status'];
        }
...
//        $where['project_nodes.status'] = array('in','0,1');
    ...
    public function submitNodes(){
        $id = input("id");
        $fields['project_nodes'] = '*';
        $fields['project_nodes_audit'] = 'id,project_nodes_id,content';
        $where['staff_id'] = session('admin')['staff_id'];
        $where['project_nodes_id'] = $id;
        $join[] = array('project','project_id','id');
        $join[] = array('project_nodes','project_nodes_id','id');
        $data = $this->loadModel("projectNodesAudit")->getJoinInfo($fields,$join,$where);

        if(input("submit")){
            $content = input("content");
            $datap['content'] = html_entity_decode($content);
            if($data['status'] == '3'){
                $this->response(INTERNAL_ERROR,'该节点已审核，请勿重复审核');
            }
            $rs = $this->loadModel('projectNodesAudit')->approveNodes($datap,$data);
            $this->ajaxOut($rs,'myProjectAudit/index');
        }
